...
| CFPS (China Family Panel Studies) | Complete (deposited waves) | 38 vars, 6 waves 2010–2022, 193,695 person-waves 16+; **family PANEL, adult frame**; all codings label-verified vs Chinese Stata; zodiac/dragon via `src/r/lookups/zodiac.R` (paper 26); no 2012 in deposit | [docs/surveys/cfps.md](docs/surveys/cfps.md) |
| Joint EVS/WVS 2017–2022 (ZA7505 v5.0.0) | Complete | 27 vars, 1 pooled cross-section, 156,658 resp, 92 country-surveys (22 CEE/post-communist); `uniqid` fully unique; ⚠️ separate from `wvs` — never row-bind (WVS7 overlaps); action_* items keep native 1=done→3=would-never (higher = withdrawal) | [docs/surveys/evs-wvs-joint.md](docs/surveys/evs-wvs-joint.md) |
...
- **ABS W5 trust levels are on a pole-merged scale — use the `*_w5_6pt` companions for W5.** W5 fielded the 13 institutional-trust items, 4 social-trust items and `econ_family_income_fair` on 6-point scales, collapsed 6→4 by merging both poles (raw 1,2→4; 5,6→1), so W5 top/bottom bins hold two native categories vs one in every other wave and W5 top-box shares/means are inflated ~2.4–4.6×. Since 2026-08-08 each item has a native W5-only companion `<id>_w5_6pt` (1–6, higher = more trust / fairer) — use it for any W5 level; use the 4-pt columns only for W4↔W6 comparisons that bypass W5. W4→W5/W5→W6 change scores on the 4-pt columns are artefactual. See docs/surveys/abs.md "W5 trust seam". **The same seam class hit the social-media block (fixed 2026-08-11)**: W5 q51a/c/d are Yes/No screeners that were fed through `safe_reverse_4pt` (No→"Sometimes", Yes→"Often") — `sm_connect_people`/`sm_share_news`/`sm_organize_politics` are now W6-only with `*_w5_binary` companions; W5 `political_info_channel` had no face-to-face option (its 5=Other now maps to 6); W4 `sm_express_political` levels need the native `sm_express_political_w4_6pt`. Anything consuming those W5 values from before 2026-08-11 is wrong. Details: docs/surveys/abs.md "Social-media format seams".
