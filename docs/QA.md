...
**One-line verdict.** The direction/reversal machinery is genuinely strong and *self-tested* — the repo ships fault-injection regression tests that reintroduce known bugs and confirm the checks flag them, and they pass. **The hard direction gate is still report-only by default** — but as of 2026-08-06 the ABS label-reconciliation backlog is **CLEARED: 0 errors** (412 ok / 0 error / 720 skip), down from 18. The remaining exposure is the 56 bin-width parity rows and, more importantly, the 720 `skip` rows: every direction bug found in the 2026-08-04/06 sweep had been sitting in that skip list, invisible. "Clean from `run_all.R`" means "no check *crashed* and no *hard* check that is *wired to fail* found something" — it does **not** mean "directionally verified." Read [§What QA does NOT guarantee](#what-qa-does-not-guarantee).
...
**Misses.** Variables whose raw labels aren't pole-classifiable → `skip`. Nominal passthroughs (no direction). Surveys/waves without raw label metadata. **`skip` is not a pass, and the skip list is where bugs hide:** ABS is 387 ok / 25 error / **721 skip**, and `govt_responds_people` sat inverted in that list until paper 01b found it empirically (2026-08-04).

✅ **The ABS W6 gap was ours, not ABS's — FIXED 2026-08-05.** This line previously read "ABS W6 has no raw label metadata for some items", which was false: all twelve W6 country `.sav` files carry full value labels, and `src/scripts/build_abs_w6.R` was stripping them before `bind_rows` (necessary — countries differ in label sets) without ever restoring them. `data/processed/w6.rds` retained labels on **0 of 488** columns, which is why 192 of the 221 `too_few_labels` skips were W6. The builder now re-attaches the labels **attribute only** (never the `haven_labelled` class) for every column whose countries agree code-for-code; where they disagree the column is left unlabelled and the conflict is reported, because a disagreement is a finding in its own right (cf. the W6 Thailand `REGION` 803/804 swap). Rebuild verified value-preserving: **0 columns changed value, 0 changed type**, 156 of 488 columns regained labels, 87 conflict, 245 were never labelled. Immediate yield: **61 skips → `ok`, 7 skips → `error`** — four extending known backlog variables into W6, and three genuinely new (`covid_livelihood_impact`, `income_fairness`, `sm_express_political`). Follow-up: the 87 conflicting columns are unverified by construction and deserve their own pass.
...
**Blind spot it inherits.** Surveys with no manifest are `SKIP`, not `FRESH`. `gcb` is currently `SKIP`.

~~And `wvs` is `STALE` **and unrebuildable**~~ — **RESOLVED 2026-07-09, re-verified 2026-08-05.** This layer found it: the manifest recorded `data/wvs/raw/wave6|7/wvs_wave6|7.parquet` as inputs, those files had been untracked away in `20a05e3`, and nothing regenerated them, so `2_harmonize_all.R` died on a missing parquet while the `.sav` it derives from sat next to it. `0_load_waves.R` now carries `.wvs_read_cached()`, which rebuilds the cache from the `.sav` on a miss. **Verified by deletion, not by reading the code:** removing each cache and re-running regenerates W6 in 5.9s (89,565 × 442) and W7 in 9.9s (97,220 × 613), both **byte-identical** to the files they replaced — 0 of 442 and 0 of 613 columns differ, and `wvs` still reports `FRESH` afterwards. The parquet is a pure derivative of the `.sav`, so it is a cache, not a source.

### 6e. Key uniqueness — 🔴 hard on the backbone, 🟡 exemption-aware on native keys
`check_key_uniqueness.R` asserts two things per survey against `data/processed/*.rds`: the engine-minted `row_uid` backbone (present, character, non-NA, unique — always an error, no exemptions exist) and the declared native key from `src/config/_audit/key_declarations.yml`, unique on complete rows up to the per-survey surplus budget in `key_uniqueness_exemptions.yml`. The same `assert_row_uid()` runs as a HARD build-time stop inside every 99-script, so a broken backbone cannot even be written.

**Claim.** A key that stops identifying rows cannot pass silently — the failure mode of the 2026-08-09 ABS `idnumber` bug (357 IDs deleted by a missing-code convention; invisible because nothing asserted uniqueness).
**Proof (injected).** `test_key_uniqueness.R` (13) + `test_row_uid.R` (10): duplicate/NA/missing row_uid → error; unexempted native dup → error; exempted dup within budget → pass; a 3-surplus violation does NOT hide behind a 2-surplus exemption; NA-key rows are skipped and counted, never asserted.
**Misses.** Native keys are asserted on complete rows only, so an ID column going all-NA reads as "0 complete rows, OK" — coverage loss is the freshness/invariant layers' job, not this one's. And row_uid stability across rebuilds is contractual (raw+loader unchanged ⇒ same IDs), not asserted per run; a raw-file revision legitimately renumbers its wave.
...
1. **A default `run_all.R` / build does not verify direction.** The gate is **report-only**; the ABS strict-reversal check is ABS-only; the label-recon and bin-width-parity checks run but their errors only *fail a run* through `run_all`'s L4_labels / L4_binwidth tallies, not through the per-build gate. **The 18-row ABS label-reconciliation backlog was cleared 2026-08-06** — all seven variables (`demo_political_equality`, `econ_family_income_fair_6pt`, `govt_should_censor_ideas`, `no_accountability_between_elections`, `gov_elections_real_choice`, `sat_president_govt`, `efficacy_ability_participate`) were triaged against their raw value labels and fixed; three needed per-wave exceptions because only some waves were inverted. **56 bin-width parity_error rows remain** (41 variables, headlined by the W5 6→4pt trust class). Separately, **6 out-of-range error rows are open across abs/wvs/afro/arab-barometer** (layer 2b) — these *do* fail `run_all`, but are not wired into the per-build gate.

   ⚠️ **The bigger exposure is now `skip`, not `error`.** ABS layer 3 reports 412 ok / 0 error / **720 skip**. Every one of the thirteen direction bugs found on 2026-08-04/06 was in that skip list — none was ever reported as an error until the lexicon or the W6 label restore reached it. A zero error count means "nothing the check could read is wrong", not "the direction is verified".
