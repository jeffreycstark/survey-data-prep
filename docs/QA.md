...
**Misses.** Variables whose raw labels aren't pole-classifiable → `skip`. Nominal passthroughs (no direction). Surveys/waves without raw label metadata. **`skip` is not a pass, and the skip list is where bugs hide:** ABS is 326 ok / 18 error / **789 skip**, and `govt_responds_people` sat inverted in that list until paper 01b found it empirically (2026-08-04).

⚠️ **The ABS W6 gap is ours, not ABS's.** This line previously read "ABS W6 has no raw label metadata for some items" — that is false. All twelve W6 country `.sav` files carry full value labels; the merged `data/processed/w6.rds` this loader reads retains them on **0 of 488 columns**, so they are dropped by our own merge step. That single defect accounts for **192 of the 221 `too_few_labels` skips** (193 distinct variables), making W6 almost entirely unverifiable for direction. Preserving labels through the merge is the highest-yield fix available to this layer. Verified 2026-08-05.
