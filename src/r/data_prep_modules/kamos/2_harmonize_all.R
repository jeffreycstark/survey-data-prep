# KAMOS: Harmonize all variables from YAML specs
#
# Sources the shared harmonization engine, then processes
# all KAMOS YAML specs from src/config/kamos/harmonize/
...
if (sys.nframe() == 0) {
  cat("\n")
  cat(strrep("=", 70), "\n")
  cat("KAMOS HARMONIZATION PIPELINE\n")
  cat(strrep("=", 70), "\n\n")
  ...
  oob_log_path <- here::here("outputs", "kamos", "oob_log.csv")
  ...
  harmonized_wide <- stack_harmonized_wide(results, waves, survey = "kamos")

  cat("\n", strrep("=", 70), "\n", sep = "")
  cat("SAVING MASTER FILES\n")
  cat(strrep("=", 70), "\n\n")

  output_dir <- here::here("outputs", "kamos")
